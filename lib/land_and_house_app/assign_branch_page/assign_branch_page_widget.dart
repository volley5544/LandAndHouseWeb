...
import 'assign_branch_page_model.dart';
export 'assign_branch_page_model.dart';

class AssignBranchPageWidget extends StatefulWidget {
  const AssignBranchPageWidget({
    super.key,
  ...
  static String routeName = 'AssignBranchPage';
  static String routePath = '/assignBranchPage';

  @override
  State<AssignBranchPageWidget> createState() => _AssignBranchPageWidgetState();
}

class _AssignBranchPageWidgetState extends State<AssignBranchPageWidget>
    with TickerProviderStateMixin {
  late AssignBranchPageModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => AssignBranchPageModel());
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if ((_model.getListChanod?.statusCode ?? 200) != 200) {
        await showDialog(
          context: context,
          builder: (alertDialogContext) {
            return AlertDialog(
              content: Text(
                  'พบข้อผิดพลาด Connection (${(_model.getListChanod?.statusCode ?? 200).toString()})'),
              actions: [
                TextButton(
      ...
      if ('${GetListChanodCall.statuslayer1(
            (_model.getListChanod?.jsonBody ?? ''),
          )?.toString()}' !=
          '200') {
        if ('${GetListChanodCall.statuslayer1(
              (_model.getListChanod?.jsonBody ?? ''),
        ...
        await showDialog(
          ...
          builder: (alertDialogContext) {
            return AlertDialog(
              content: Text('พบข้อผิดพลาด (${GetListChanodCall.statuslayer1(
                (_model.getListChanod?.jsonBody ?? ''),
              )?.toString()})'),
              actions: [
                TextButton(
      ...
      _model.chanodlist = GetListChanodCall.assignForData(
        (_model.getListChanod?.jsonBody ?? ''),
      )!
          .toList()
          .cast<ChanodData3Struct>();
      safeSetState(() {});
      _model.chanodlistto = GetListChanodCall.assignTodData(
        (_model.getListChanod?.jsonBody ?? ''),
      )!
          .toList()
          .cast<ChanodData3Struct>();
